...
-- LEFT JOIN을 하여 import된 두 테이블 간의 데이터 유효성 검증
-- 학생 Table에 없는 학과코드가 있는지 검증하기
-- 학생 Table에서 학과 Table 간의 FK 설정을 하기 위한 검증
...
-- 과목명 칼럼에 국어 문자열이 담겨 있으면 해당 레코드의 점수 칼럼 값을 표시하고 그렇지 않으면 (null)을 표시하라
SELECT 학번,
        DECODE(과목명, '국어', 점수) AS 국어점수,
...
-- 위의 SQL을 학번으로 GROUPING하고 각 점수를 합산(SUM())하면 DBMS의 SQL에서는 (null) + 숫자 = 0 + 숫자와 같다
