...
-- 1년동안 매입과 매출금액을 계산하고 단순 이익금 계산해보기
SELECT
...
-- 매입매출관련하여 소매점에서 상품을 매입하여 소비자한테 판매할 때
-- 매입할 때 매입부가세 발생
-- 판매할 때 매출부가세 발생
-- 매출부가세 - 매입부가세를 계산하여 1년에 2 ~ 4회에 부가가치세를 납부한다

-- 농사를 지어서 쌀 20KG를 생산하여 판매를 하면 5만원 정도의 금액에 판매하게 된다.
-- 쌀을 공장에서 가공하여 생산품(공산품)으로 만들게 되면 실제 20KG의 쌀을 직접 판매하는 것보다 더 비싼 가격에 판매하게 된다
-- 이 때 실제 쌀보다 더 많은 이익이 발생하게 되므로 '가치가 부가되었다'라고 표현한다.
-- 가치가 부가된 만큼 세급을 납부하도록 한다
...
-- 매출금액은 부가세 10%를 포함한 금액으로 입력한다
-- 샘플데이터의 매입금액은 VAT 제외금액이고 매출금액은 VAT 포함된 금액이다.
-- 매입과 매출 데이터로 지난 1년간 납부한 VAT 금액을 계산해보자

-- 매입금액 : 22737397
-- 매출금액 : 41683800
-- VAT = (매출금액에 포함된 VAT) - (매입금액 * 0.1)
-- 매출금액의 VAT 제외된 함계 = 매출금액 / 1.1
...
- SUM(DECODE(io_inout, '매입', ROUND(io_total * 0.1))) AS 매출부가세
FROM tbl_iolist;
...
-- 2021-01-01부터 2020-06-30 기간동안 거래된 리스트를 거래처별로 조회
...
-- 같은 상품이라도 거래 시기에 따라 매입과 매출금액이 달라질 수 있기 때문에 데이터 중에서 제일 높은 단가를 가져오기
SELECT io_pname,
...
E2 항목의 값을 0.8을 곱하면 E2의 80% 가격이 되고 다시 그 금액을 1.1로 나누면 부가세를 제외한 가격이 된다.
=ROUND(IF(C2 = 0, ((E2 * 0.8)/1.1), C2),0)

5. 매출단가가 0인 데이터는 매입데이터에서 임의로 생성하기
매입단가의 20%를 추가하고 부가세 10%를 추가
        매입단가 + 20% + 10%
=INT(IF(E2=0, (D2 * 1.2) * 1.1, E2) / 10) * 10
*/
...
-- DEFAULT 값 지정하지 않으면 자동으로 NOT NULL로 설정
...
/*
매입매출 데이터로부터 상품정보, 거래처정보 데이터를 생성하고 테이블을 생성하여 데이터를 import했다

매입매출데이터와 상품정보, 거래처정보를 JOIN하기 위해서는 매입매출데이터에 상품코드, 거래처코드가 있어야 한다.
그러나 현재 데이터에는 코드 칼럼 없이 이름 칼럼만 있는 상태이다

매입매출데이터에 상품코드, 거래처코드 칼럼을 추가하고 3개 Table을 JOIN할 수 있도록 변경하기
*/