...
-- 표준 SQL에서는 SELECT 연산식 형식으로 수행해볼 수 있는데
-- Oracle에서는 FROM절이 없는 SELECT를 허용하지 않는다.
-- DUAL이라는 DUMMY TABLE을 준비해놓고
-- SQL 연산을 수행할 때 사용하도록 만들어 놨다.
...
-- 실제 날짜값으로 변환하여 표현하기
-- 문자열로 되어있는 날짜 시간을 실제 Data Type의 데이터로 변환
...
-- 강제 형변환이 다소 불편할 수 있지만 결과를 정확히 예측할 수 있기 때문에 자동 형변환보다는 권하는 방식이다.
