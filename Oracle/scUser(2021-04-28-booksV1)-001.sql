...
-- TABLE을 생성하고 데이터가 있는 상태에서 PK를 변경, 추가하는 경우에는 PK로 설정하려고 하는 칼럼의 데이터가 PK조건(유일성, NOT NULL)을 만족하지 않는 데이터가 있으면 명령이 실패한다
-- 대량의 데이터가 저장된 TABLE을 변경할 경우는 매우 신중하게 실행을 해야 한다
-- 또한 미리 데이터 검증을 통하여 제약 조건에 위배되는 데이터가 있는 지 확인을 해야 한다
ALTER TABLE tbl_books -- tbl_books table을 변경
ADD CONSTRAINT PK_ISBN -- 제약조건을 추가하는 데 이름을 PK-ISBN
...
도서정보는 어플로 만들기 전에 사용하던 데이터인 관계로 데이터베이스의 규칙에 다소 어긋난 데이터가 있다

저자 항목(칼럼)을 보면 저자가 2명 이상인 데이터가 있고 또한 역자도 2명 이상인 경우가 있다

데이터를 저장할 칼럼을 크게 설정하여 입력(import)하는 데는 문제가 없는 데 저자나 역자를 기준으로 데이터를 여러가지 조건을 부여하여 조회를 하려고 하면 문제가 발생할 수 있다
...
RENAME COLUMN bk_author TO bk_author1; -- bk_author 칼럼을 bk_author1으로 변경
...
저자 데이터를 저장할 칼럼이 이후에 또 변경해야하는 상황이 발생할 수 있는 이슈가 발견되었다

제2정규화를 통하여 테이블 설계를 다시 해야하겠다
1. 정규화를 수행할 칼럼이 무엇인가 파악(인식)한다
    저자 데이터를 저장할 칼럼
    복수의 데이터가 필요한 경우
...
-- tbl_books 테이블의 데이터를 삭제하고 제1정규화가 완료된 데이터로 다시 import
...
WHERE bk_author2 IS NOT NULL -- bk_author2 칼럼에 데이터가 있는 경우만 나타내라
GROUP BY '(' || bk_isbn, bk_author2;

-- 도서정보와 저자리스트를 JOIN하여 데이터 조회
--저자가 1명인 경우는 1개의 도서만 출력이 되고
-- 저자가 2명 이상인 경우는 같은 ISBN, 같은 도서명, 다른 저자 형식으로 저자 수만큼 데이터가 출력된다
SELECT bk_isbn, bk_title, ba_author
FROM tbl_books
    LEFT JOIN tbl_book_author
        ON bk_isbn = ba_isbn;

--제2정규화가 완료된 상태에서 도서정보를 입력하면서 저자정보를 추가하려면 저자정보에는 ISBN 저자명을 포함한 데이터를 추가해주면 된다
...
-- tbl_book_author에 데이터를 추가하려고 할때 9791162540572 이 도서에 저자를 추가하고 싶을 때 
-- 테이블의 ba_seq 칼럼에는 이미 등록된 값이 아닌 새로운 숫자를 사용하여 데이터를 추가해야 한다.
