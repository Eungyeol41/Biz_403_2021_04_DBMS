...
-- 위에서 생성한 tbl_student 테이블에는 데이터를 추가하려고 할 때
-- 이름 데이터가 없어도 데이터가 정상적으로 추가가 되어버린다
-- 같은 학번의 데이터가 이미 추가되어 있어도 또 다시 추가가 되어버린다
-- 이런 식으로 데이터가 계속 추가된다면 전체 데이터의 신뢰성에 문제가 될 것이다
-- DBMS에서는 table(Entity)를 설계할 때 이러한 오류를 방지하기 위하여 Table을 생성할 때 '제약 조건'을 설정하여 데이터가 INSERT되지 못하도록 하는 기능이 있다.
-- 작성된 table을 삭제하고 다시 제약 조건을 설정하여 생성하자

DROP TABLE tbl_student CASCADE CONSTRAINTS;

-- 1. 학생의 이름을 데이터가 반드시 있어야만 한다
...
-- 학번 칼럼이 NOT NULL되어 있어서 INSERT 불가
...
-- 기본키는 제약 조건에 UNIQUE와 NOT NULL을 같이 설정해야 하는데, DBMS에서는 기본키 제약 조건을 설정하는 키워드가 별도로 있다
...
INSERT INTO tbl_student (st_num, st_dept, st_name)
VALUES ('00100', '법학과', '성춘향');

INSERT INTO tbl_student (st_num, st_dept, st_name)
VALUES ('00002', '컴퓨터공학', '홍길동');
SELECT * FROM tbl_student;
