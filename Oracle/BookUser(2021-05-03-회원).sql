...
-- tbl_buyer 데이터 1개(1명)의 고객이 다수수의 tbl_book_rent table에 포함이 된다
-- tbl_buyer 1 : tbl_book_rent N = 1 : 다의 관계이다
-- N의 table에서 FK 설정을 하고, 1의 table과 관계를 맺는다
ALTER TABLE tbl_book_rent -- N(多) Table
...
tbl_book_rent table에는 필수적으로 사용하는 데이터 칼럼을 PK로 설정하는 데 어려움이 있다.
다른 table의 칼럼을 참조를 하고는 있지만 모든 칼럼이 중복 값을 가질 수 있는 관계로 단일 칼럼으로 PK를 설정할 수 없다

PK를 설정하고, 더불어 주문관련 리스트를 보는 데 사용할 br_seq 칼럼을 만들고 이 칼럼으로 PK를 설정했다

book_rent table에 데이터를 INSERT하려고 할 때 항상 유일한 값으로 PK를 설정해야하는 어려움이 있다
보통 이러한 칼럼은 일련번호 순으로 만드는 데 오라클 이외의 다른 DBMS에는 일련번호를 자동으로 만들어주는 Table 속성이 있다
오라클(11 이하)에서는 그러한 속성이 없다

mySQL의 경우 칼럼 속성에 AUTO_INCREMENT로 설정을 하면 그 칼럼은 INSERT할 때 별도 값을 지정하지 않아도 항상 유일한 일련번호로 자동 생성이 된다

오라클에서는 일련번호를 생성하기 위한 별도의 객체가 존재한다
*/
-- 오라클에서 일련번호를 생성하는 SEQUENCE 객체
-- DDL 명령을 사용하여 SEQ 객체 생성
-- 이름을 명명할 때 SEQ_로 시작하고 뒤에 적용할 대상 table명을 붙여 생성한다
...
-- SEQ의 START 값을 적용하는 테이블의 SEQ의 최대값보다 크게 설정을 하자
CREATE SEQUENCE seq_book_rent
START WITH 11
INCREMENT BY 1;

INSERT INTO tbl_book_rent(br_seq, br_sdate, br_isbn, br_bcode)
VALUES(SEQ_BOOK_RENT.nextval, '2021-04-20', '9791188850303', 'B0020');
...
-- FK 관계가 설정되지 않은 TABLE 간에서는 조회되는 데이터가 실제와 다를 수 있다
SELECT 
    BR.br_sdate AS 대여일,
    BR.br_bcode AS 회원코드,
    BU.bu_name AS 회원명,
...
-- LEFT (OUTER) JOIN
-- 테이블 간에 FK가 설정되지 않고 참조하는 테이블의 데이터가 마련되지 않은 경우 FROM 절에 표현된 Table 데이터 위주로 조회하고자 할 때 
...
-- where 조건에 맞는 데이터를 골라서 그룹으로 묶는다
...
-- 직업별로 급여를 합계 계산할 때 직업이 영업직인 사람을 제외하고 싶다
SELECT 직업, SUM(급여)
FROM tbl_급여
WHERE 직업 != '영업직'
GROUP BY 직업;

-- 급여테이블에서 영업직을 제외한 직업의 급여 총합계를 계산하고 총 합계가 3000000 이상인 데이터만 보여라
SELECT 직업, SUM(급여)
FROM tbl_급여
WHERE 직업 != '영업직'
GROUP BY 직업
...
-- 급여테이블에서 영업직을 제외하고 실급여가 1000000을 초과하는 데이터만 합산하여 보여라
SELECT 직업, SUM(급여)
FROM tbl_급여
...
VALUES (seq_book_rent.NEXTVAL, '2021-04-01', 'B0011', '9791162540671');

-- 대여일이 2021-04-25일 이전이고 미반납된 대여 정보를 보여라
-- 그리고 회원코드순으로 보여라
SELECT 대여일, 회원코드, 회원명, 회원연락처, 도서명
FROM view_도서대여정보
WHERE 대여일 < '2021-04-25' AND 반납일 IS NULL
