...
-- 학번으로 묶어서, 동일한 학번의 국어점수의 합계를 할 때
...
-- 성적데이터를 보면서 각 학생의 이름 등을 같이 보고싶다
-- 2개의 테이블을 JOIN하여 함께 보자
-- tbl_score 테이블을 나열하고 tbl_score의 sc_num 칼럼의 값과 같은 데이터를 tbl_student에서 찾아서 함께 나열하라
SELECT *
...
-- 2개 이상의 테이블을 JOIN 할 때 각각 테이블의 칼럼(속성) 이름이 같은 경우 문제가 발생할 수 있다.
...
-- 테이블 이름을 부착하기가 번거로우면 테이블에 Alias를 추가한 후 각 칼럼 이름에 Alias를 사용할 수 있다
-- 테이블 Alias에도 AS 키워드를 사용하지만 오라클에서는 테이블 Alias에 AS 키워드를 사용하면 문법 오류가 발생한다.
SELECT SC.sc_num,
    ST.st_name,
    ST.st_dept,
    SC.sc_kor,
...
-- 학생데이터에서 일부를 삭제한 후 JOIN을 실행하였더니 성적데이터가 79개 밖에 조회되지 않는다.
-- 성적데이터는 모두 100개가 있지만 학생데이터는 79개만 남아있기 때문에 JOIN한 결과가 학생데이터와 같은 수인 79개만 조회(인출)되고 있다.
...
-- 학생데이터에는 1 ~ 79번까지만 있고 성적데이터에는 1 ~ 100까지 있다.
-- 성적데이터의 80번부터 100번까지는 실제로 존재하는 학생인지 아닌지 증명할 방법이 없다.
-- 성적데이터는 무결성이 깨진 상태가 된다.
-- 학생테이블과 성적테이블 간의 연관(관계) 참조가 무너진 상태가 된다
-- '== 참조 무결성이 오류가 발생했다'라고 표현한다.
-- 참조 무결성에 문제가 생긴 경우 JOIN을 했을 때 인출되는 데이터의 신뢰성을 보증할 수 없다.
SELECT * FROM tbl_score;
SELECT * FROM tbl_student;

-- 참조 무결성 여부와 관계없이 모든 데이터를 JOIN하여 보고 싶을 때 LEFT JOIN(LEFT OUT JOIN)
--  tbl_score 테이블의 데이터는 모두 보여주고 학생테이블에서 학번이 일치하는 학생이 있으면 같이 보여달라
-- 보통 table의 참조 무결성 보증을 설정하는 경우가 있는데 LEFT JOIN을 사용한다.
-- 참조 무결성의 조건이 
SELECT SC.sc_num,
    ST.st_name,
    ST.st_dept,
...
-- EQ JOIN을 실행할 때 조건을 부여하지 않으면 테이블 X 테이블만큼의 데이터가 출력된다
-- 이렇게 인출된 데이터를 '카티션곱'이라고 한다.
SELECT COUNT(*)
FROM tbl_score, tbl_student;

-- 학생데이터에 없는 학생의 성적이 추가되어 있는 지 여부를 알아보고 싶을 때 참조 무결성에 오류가 있는 지 알고 싶을 때
SELECT COUNT(*)
FROM tbl_score
...
-- 학생데이터와 성적데이터 간의 참조 무결성에 오류가 있기 때문에 실제 학생데이터에 존재하는 성적정보만 보고 싶을 때
SELECT COUNT(*)
FROM tbl_student
    LEFT JOIN tbl_score
        ON st_num = sc_num;
...
-- 학과순으로 정렬하고 같은 학과가 있으면 다시 총점 순으로 정렬
SELECT * FROM view_성적정보
ORDER BY 학과, 총점 DESC;
