...
USE myDB;
-- 현재 DB(myDB)에 있는 모든 Table을 보여라
SHOW TABLES;

-- MySQL에서는 일련번호와 관련된 칼럼에 auto_increment 옵션을 설정하면 INSERT할 때 값을 지정하지 않아도 자동으로 ID, SEQ 값을 생성하여 칼럼에 추가하여 준다.
...
-- MySQL은 Linux 철학을 유지하고 있기 때문에 명령이 정상적으로 수행되면 아무런 메시지도 보이지 않는다.
